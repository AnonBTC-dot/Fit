      ...
      <div className="grid grid-cols-2 gap-3">
        {PRESET_PROFILES.map((p) => (
          <button
            key={p.slot}
            onClick={() => pick(p.slot)}
            className="rounded-xl2 border border-ink-100 bg-white p-6 text-center shadow-sm transition-all active:scale-[0.96]"
          >
            <div className="mx-auto mb-2 flex h-16 w-16 items-center justify-center rounded-full bg-brand-50 text-3xl">
              {p.emoji}
            </div>
